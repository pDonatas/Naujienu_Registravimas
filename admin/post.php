...
if(isset($_POST['ok'])){
    ...
    if(isset($_POST['comment']))
        $comment = 1;
    $query = $pdo->prepare("INSERT INTO `news` (`title`, `text`, `comment_status`, `fk_USERid`, `status`,`created_at`,`updated_at`) VALUES (?, ?, ?, ?, '1',?,?)");
    $query->execute([
        $_POST['title'],
        $_POST['text'],
        $comment,
    ...
    if($_POST['media'] > 0){
        for($i = 1; $i <= $_POST['media']; $i++){
            ...
            if(move_uploaded_file($_FILES['media'.$i]["tmp_name"], "../img/media/".$filename)){
                ...
                $query->execute([$id]);
                die("Nepavyko įkelti failų, todėl naujienos kėlimas sustabdytas: ".$_FILES["media".$i]["error"]);
            }
        }
    }
